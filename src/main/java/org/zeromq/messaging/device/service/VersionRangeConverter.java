...
public final class VersionRangeConverter implements ObjectAdapter<String, byte[]> {
  ...
  public byte[] convert(String src) {
    Matcher matcher = PATTERN_VERSION_RANGE.matcher(src);
    Preconditions.checkState(matcher.matches(), "wrong version_range format: " + src);
    return ZmqUtils.mergeBytes(ImmutableList.of(versionAsBytes(matcher.group(1)/* "from" version */),
                                                versionAsBytes(matcher.group(2)/* "to" version */)));
  }
